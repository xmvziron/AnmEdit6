...
AnmManager::AnmManager()
{
    ...
    std::memset((void *) this, 0, sizeof(AnmManager));
...
void GetTexturePath(char *buf, size_t size, const char *path, const char *anmName)
{
    /* since we don't rely on the asset pack, we need to figure out the
     * location of the texture, assuming it's in the same folder
     */
    ...
    /* TODO: we assume only one kind of path */
    if (strchr(anmName, '/') != NULL)
    {
        separator = "/";
    }
    else if (strchr(anmName, '\\') != NULL)
    {
        separator = "\\";
    }
    ...
    const char *fileName = anmName;
    const char *separatorLoc = fileName;

    /* find the base path */
    while (true)
    {
        separatorLoc = strchr(separatorLoc, separator[0]);

        /* we've hit the last separator */
        if (separatorLoc == NULL)
        {
            break;
        }
        ...
        fileName = separatorLoc;
    }

    char textureFolder[MAX_PATH + 1] = {};

    strncpy(textureFolder, path, sizeof(textureFolder));
    
    separatorLoc = textureFolder;

    char *lastSeparatorLoc = NULL;
    ...
    while (true)
    {
        ...
        if (separatorLoc == NULL)
        {
            /* remove everything after last separator -> dirname */
            lastSeparatorLoc++;
            *lastSeparatorLoc = '\0';
            break;
        }
        
        lastSeparatorLoc = (char *) separatorLoc;
        separatorLoc++;
    }


    snprintf(buf, size, "%s%s", textureFolder, fileName);
}
...
ZunResult AnmManager::LoadAnm(i32 anmIdx, const char *path, i32 spriteIdxOffset)
{
    ...
    char texturePath[MAX_PATH + 1];
    
    GetTexturePath(texturePath, sizeof(texturePath), path, anmName);
    ...
    else if (this->LoadTexture(anm->textureIdx, texturePath, anm->format, anm->colorKey) != ZUN_SUCCESS)
    {
    ...
    if (anm->alphaNameOffset != 0)
    {
        ...
        if (this->LoadTextureAlphaChannel(anm->textureIdx, texturePath, anm->format, anm->colorKey) != ZUN_SUCCESS)
        {
            GameErrorContext::Fatal(&g_GameErrorContext, TH_ERR_ANMMANAGER_TEXTURE_CORRUPTED, anmName);
            return ZUN_ERROR;
        }
